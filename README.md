...
`tools/list` at `https://gateway.pipeworx.io/ens/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/resolve_ens \
  -H 'Content-Type: application/json' \
  -d '{"name":"vitalik.eth"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/resolve_ens`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
